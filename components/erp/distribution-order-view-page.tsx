...
type DistributionOrderViewPageProps = {
  orderId: string;
};
...
export default function DistributionOrderViewPage({ orderId }: DistributionOrderViewPageProps) {
  ...
  const submitPayment = async () => {
    ...
    setMessage(null);
    try {
      const response = await fetch(`${ERP_API_BASE}/distribution/orders/${orderId}/payments`, {
        method: "POST",
        headers: {
      ...
      const refresh = await fetch(`${ERP_API_BASE}/distribution/orders/${orderId}`, {
        headers: {
          Accept: "application/json",
          Authorization: `Bearer ${token}`,
        },
  ...
  return (
    <div className="space-y-6">
      <ErpPageHeader
        title={order ? order.order_number : "Distribution Record"}
        description="Review what the distributor took, how much was paid, what is left, and add new balance payments."
        action={
          <div className="flex flex-wrap gap-3">
            ...
            <Button asChild className="rounded-xl bg-slate-950 text-white hover:bg-slate-800">
              <Link href={`/dashboard/distribution/orders/${orderId}/edit`}>
      ...
      {loading ? <p className="text-sm text-slate-500">Loading distribution record...</p> : null}
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full text-sm">
                ...
                <tbody className="bg-white">
                  <tr>
                    <td className="px-4 py-4 font-medium text-slate-900">{order.distributor_name}</td>
                    <td className="px-4 py-4">{Number(order.bottles_issued ?? 0).toLocaleString()}</td>
                    <td className="px-4 py-4">{order.selected_batch_number || order.batch_sources || "-"}</td>
                    <td className="px-4 py-4">₦ {Number(order.amount_paid ?? 0).toLocaleString()}</td>
                    <td className="px-4 py-4">₦ {Number(order.balance_due ?? 0).toLocaleString()}</td>
                  </tr>
                </tbody>
              </table>
            </div>
              ...
              <table className="min-w-full text-sm">
                ...
                <tbody className="divide-y divide-slate-200 bg-white">
                  {(payload?.allocations ?? []).length === 0 ? (
                    ...
                    (payload?.allocations ?? []).map((allocation: any, index: number) => (
              ...
              <table className="min-w-full text-sm">
                ...
                <tbody className="bg-white">
                  <tr>
                    ...
                    <td className="px-4 py-4">₦ {Number(order.unit_price ?? 0).toLocaleString()}</td>
                    <td className="px-4 py-4">₦ {Number(order.total_amount ?? 0).toLocaleString()}</td>
                    <td className="px-4 py-4">₦ {Number(order.amount_paid ?? 0).toLocaleString()}</td>
                    <td className="px-4 py-4">₦ {Number(order.balance_due ?? 0).toLocaleString()}</td>
